...
export const make_tex = (gl, dims, ifrmt=gl.RGBA32F, frmt=gl.RGBA, dtype=gl.FLOAT) => {
  ...
  gl.texImage2D(type, 0, ifrmt, ...dims, 0, frmt, dtype, null);
...
export const upload_arr = (gl, arr, buf, loc=0, dim=3, type=gl.FLOAT, hint=gl.STREAM_DRAW) => {
  gl.bindBuffer(gl.ARRAY_BUFFER, buf);
  gl.enableVertexAttribArray(loc);
  if (type == gl.FLOAT) gl.vertexAttribPointer(loc, dim, type, false, 0, 0);
  if (type == gl.INT)   gl.vertexAttribIPointer(loc, dim, type, 0, 0);
