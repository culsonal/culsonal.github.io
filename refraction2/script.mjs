...
const gl = gpu.good_defaults_gl(canvas, false);
...
const prog = gpu.make_frag_prog(gl, gl.canvas.width, `
  ...
  bool is_inside_sphere(vec3 pt) {
    return length(pt - sphere_xyz) < 1.8;
  }
  ...
  out vec3 frag_color;
  void main() {
    // this should be different for r/g/b
    //float index_of_refraction = .1;

    vec2 xy = gl_PointCoord.xy*2.-1.;
    vec3 pt_on_camera_plane = vec3(xy, -2);
    ...
    for (int i = 0; i < max_num_iters; i++) {
      ...
      if (is_inside_sphere(pt) && !is_inside_sphere(pt_new) ||
        ...
        ray_dir = normalize(refract(ray_dir, sphere_normal(pt), index_of_refraction));
      }
...
let itr = 0; const loop = () => { itr += 1; window.requestAnimationFrame(loop);
  ...
  const index_of_refraction = .1;

  gpu.run_prog_pts(gl, prog, 1, {
    index_of_refraction,
    camera_xyz: [ 0, 0, -3 ], //[ Math.sin(itr/200)*.4, Math.cos(itr/150)*.4, -3 ],
    sphere_xyz: [ Math.sin(itr/100)*.2, 0, Math.cos(itr/100)*.6 ]
  });
