...
const draw_pts = gpu.make_prog(gl, `${await (await fetch('hash.c')).text()}
  ...
  float implicit_intersection(float f1, float f2) {
    return max(f1, f2);
  }
  ...
  float implicit_fn(vec2 pt) {
    ...
    float ay = .4;
    float lens_a = implicit_intersection(
      implicit_ellipse(pt-vec2(0, ay+.4), vec2(.5, .5)),
      implicit_ellipse(pt-vec2(0, ay-.2), vec2(.2, .2))
    );

    float by = -.1;
    float lens_b = implicit_intersection(
      implicit_ellipse(pt-vec2(0, by+.4)-obj_center, vec2(.6, .6)),
      implicit_ellipse(pt-vec2(0, by-.4)-obj_center, vec2(.6, .6))
    );

    return implicit_union(lens_a, lens_b);
  }
