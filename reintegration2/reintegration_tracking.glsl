...
particle unpack_particle(ivec2 xy) {
  ...
  return particle(
    ...
    unpackHalf2x16(floatBitsToUint(t.w))
  );
}
...
vec4 pack_particle(particle p) {
  return vec4(
    ...
    uintBitsToFloat(packHalf2x16(p.orig))
  );
}
...
particle advect_particle(ivec2 ixy, float dt) {
  particle p_next = particle(0., vec2(0), vec2(0), vec2(0));
  ...
  for (int dx = -2; dx <= 2; dx++) for (int dy = -2; dy <= 2; dy++) {
    ...
    p_next.orig += np.orig*overlap_mass;
  }

  if (p_next.mass > 0.) {
    p_next.vel /= p_next.mass;
    p_next.cm  /= p_next.mass;
    p_next.orig /= p_next.mass;
  }
...
void update_via_orig(inout particle p, ivec2 ixy, float dt) {
  vec2 diff = vec2(ixy) + p.cm - p.orig;
  p.vel -= .1*dt*diff;
  p.orig = mix(vec2(ixy)+p.cm, p.orig, .5);
}
...
particle update_particle_physics(ivec2 ixy, float dt) {
  ...
  if (distance(vec2(ixy)/vec2(DIM), mouse.xy) < .05) { p.vel += mouse.zw*10.; p.mass += .002; }
...
void draw() {
  ...
  //out_col = vec4(vec3(distance(vec2(ixy)+p.cm, p.orig)*.1, p.mass, p.mass), 1);
